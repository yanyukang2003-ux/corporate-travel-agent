...
- **§30.10 多城方案六步** —— 01–05 已落地（见 §32–§36），**只剩第 06 步：一次多段报价**
...
| **多城（三段以上）** | ✅ §36 通了。一句「9月15号从北京去上海开会，9月18号去杭州见客户，9月19号回北京」端到端出三段方案，真实模型 MT-03 通过。**但每段是单独问价（= N 张单程票），价格大概率偏高**——第 06 步在解，见 §36.7 |
...
```
pytest 719 / ruff（src tests examples migrations）   全过（2026-08-29，§36）
...
多轮 + 多段行程 · 真实模型 · 每条 2 次                 5/5   ← prompt v13d（含多城 MT-03）
多城航段抽取 · 真实模型 · 每条 3 次                   段数/路线/顺序/住宿 21/21
```

**未在 v13d 下重跑，引用前请注意：**
- LLM Judge 60 条（v11 下均分 4.55、邻近一致 97.6%；已隔了 v12、v13 两轮提示词改动）
...
| 06 | **一次多段报价（Duffel slice）** | ⬜ 下一步 | 现在每段单独问价 = N 张单程票，价格大概率偏高。**开工前先量一次价差**，见 §36.7 |
...
*本轮见 §36（语义层出航段列表，多城打通）；上一轮 §35（城市登记表）；再上 §34、§33、§32、§31；再上 §29；再上 §28、§27、§26、§25；A–F 见 §19；G 见 §20；H 见 §21；I 见 §22。*
