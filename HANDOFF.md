...
**最新一轮见 §27：LLM Judge 第一次真跑（均分 4.55）；语义任务在真 Postgres 上跑通并
抓到一个「发一次版就锁死所有在途任务」的严重 bug；双 worker 并发验收 2/2。
`pytest 634`。分支 `semantic-entrypoint-and-judge`，工作树干净。**
...
*交接更新 2026-08-28（第二轮）。本轮见 §27（政策哈希跨进程 bug / Postgres 语义端到端 /
双 worker 并发 2/2 / Judge 真跑 + 单标注者模式）。*
*沟通标准见 `AGENTS.md`：先解释名词再用，先给结论再给细节，诚实优先于漂亮。*
*上一轮见 §26；再上 §25、§24、§23；A–F 见 §19；G 见 §20；H 见 §21；I 见 §22。*
