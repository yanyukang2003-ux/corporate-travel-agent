...
**架构方案与六步计划见 §30，六步已全部落地**（01–03 见 §29，04–06 见 §31）。
**下一步不是继续往下加，是先花钱重跑真实模型验证**——提示词改了，旧分数已作废，见 §31.6。
...
|---|---|
| 单程、原路往返 | ✅ |
| **开口程**（去上海、从杭州回） | ✅ §29 支持（真实模型 2/2 是 v11 下的成绩，同样待重跑） |
| 多城（三段以上） | ❌ 领域模型能表达（上限 6 段），但**语义层还只产出 1–2 段**；会被正确拦住不搜 |
| 多轮对话逐步搭行程 | ⚠️ 曾经真实模型 5/5，但提示词已改（v11→v12），**分数作废待重跑** |
| 中文日期边角 | ⚠️ 曾经真实模型 8/8，同上，**待重跑** |
...
```
pytest 675 / ruff / 前端 build+test+lint     全过（2026-08-29）
红队 runner（语义入口）                       17/17
冻结数据集 1.0.3 deterministic_live           对得上，数据未改动

以下均在 prompt v11 下测得，v12 后作废，重跑前不要引用：
日期边角 · 真实模型 · 每条 3 次                8/8
多轮 + 多段行程 · 真实模型 · 每条 2 次          5/5
LLM Judge 真跑 60 条                          均分 4.55，邻近一致 97.6%
```

Postgres 双 worker 并发（不调模型，不受提示词影响）：本轮**未跑**，需要本机起 Postgres。

### 1.5 待项目所有者拍板（按紧急程度）

0. **要不要花钱重跑真实模型评测** —— 提示词 v11→v12，上面那几个分数现在**全部作废**。
   不重跑就不知道 04–06 有没有引入退步。这是最紧急的一条，详见 §31.6
1. **城市别名表补全** —— 已经第三次咬人（大阪、广州、杭州）。`config/travel-policy.json`
   只有 10 个城市，表外城市**半英半中地继续跑且不报错**，真实 Provider 能否查询未验证。
   §31 第 06 步的「两个名字是同一座城市就不问」**完全依赖这张表**，表外城市照旧白问一次
...
下一步该做什么见 §31.6——**不是继续往下加，而是先验证 04–06 在真实模型上成立**。
...
```
pytest 675（04 新增 10、05 新增 6、06 新增 13）   全过
...
**没跑：** 真实计费模型（多轮 / 日期边角 / Judge）、Postgres 双 worker。
...
1. **先验证，别再往下加。** 04–06 改的都是"系统怎么决定"，而这些决定此前有真实模型
   跑分（多轮 5/5、日期 8/8）。提示词已经改了，**这些数字现在全部作废**，
   要新开报告目录重跑才知道有没有退步。这是花钱的事，需要项目所有者点头。
...
*本轮见 §31；上一轮 §29；再上 §28、§27、§26、§25；A–F 见 §19；G 见 §20；H 见 §21；I 见 §22。*
