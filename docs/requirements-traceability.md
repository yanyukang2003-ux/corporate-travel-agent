...
| 基线要求 | 当前实现 | 位置 | 状态 |
|---|---|---|---|
| 单 Agent 编排器 | 显式工作流控制器 | `agent/orchestrator/` | 已实现 |
...
| 供应商失败安全降级 | 3 次即时尝试、60 秒熔断、`WAITING_FOR_PROVIDER` 与最多 3 次持久化延迟重试；耗尽进入 `PROVIDER_FAILED` | `agent/orchestrator/`, `services/provider_resilience.py` | 已实现 |
| 分布式延迟重试所有权 | PostgreSQL 原子 claim、lease expiry 接管、fencing token 写回与专用 worker 角色 | `services/sqlalchemy_repository.py`, `agent/orchestrator/`, `api/main.py` | 已实现；PostgreSQL 并发压测待生产环境留证 |
| 例外审批暂停/恢复 | 绑定主题哈希和指定审批人 | `agent/orchestrator/` | 已实现 |
| 价格/库存重验 | 选择或批准后强制 revalidate | `agent/orchestrator/` | 已实现 |
...
| 幂等 Booking Intent | 由任务/请求/方案版本生成键 | `agent/orchestrator/` | 已实现 |
| 审计轨迹 | 状态、快照、规则、审批和重验事件 | `services/audit.py` | 已实现（内存/PostgreSQL） |
| 自然语言理解 | 工具循环：模型把对话翻译成带原话证据的工具参数，宿主逐项校验 | `agent/tool_loop.py`, `agent/tool_loop_adapter.py` | 已实现；真模型评测见 `reports/evaluation-runs/toolloop-*`、`external-longtail-*` |
| 最多三轮澄清 | 跨轮字段合并、确定性问题、结构化降级 | `agent/orchestrator/` | 已实现 |
| 每任务最多 12 次工具调用 | 统一预留、成功/失败计数、审计与终止态 | `agent/orchestrator/`, `domain/models.py` | 已实现 |
| Prompt Injection 边界 | 工具白名单无写操作、交付只能引用真实 `ref_id`、对抗集 CI 门禁 | `agent/tool_loop.py`, `evaluation/adversarial.py`, `tests/test_adversarial_tool_loop.py` | 已实现 |
...
| 无可行方案可解释 | 区分去程、返程、酒店无库存与硬过滤 | `agent/orchestrator/` | 已实现 |
| 库存与交接有效期 | 共享时钟、快照/链接过期拦截与审计 | `providers/mock.py`, `agent/orchestrator/` | 已实现 |
