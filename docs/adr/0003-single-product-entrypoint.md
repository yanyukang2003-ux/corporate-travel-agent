...
|---|---|---|
| Legacy real-model harness (`evaluation_model_runner`, `evaluation_model_live_provider`, `evaluation_model_preflight`; D9–D14 runners) | built on `create_task_from_message`; porting meant re-baselining six frozen real-model sets | live tool-loop runs under `reports/evaluation-runs/toolloop-*` and `agentic-boundary-*`, driven by `examples/run_tool_loop_*` and `run_agentic_boundary_longtail_evaluation.py`. **Lost** at the time: the protocol §5 trace JSONL, price-table cost accounting and the explicit retry-cap regressions (D10/D11) for real-model runs. **Rebuilt 2026-09-01** as `services/evaluation_tool_loop.py` (`LiveRunLedger` / `LiveCaseRecorder`): every tool-loop runner now writes `traces.jsonl`, `cost-ledger.jsonl`, `cost-summary.json` and `retry-cap-check.json`; the deterministic 429/402 regression lives in `tests/test_evaluation_tool_loop.py`. |
| Adversarial suite (`evaluation_adversarial`, `adversarial-v1` runner) | its compromised model returned legacy extraction payloads | host invariants it verified are unit-tested on the product path: invented references rejected, no write tool reachable, policy verdicts computed by code (`tests/test_tool_loop.py`, `tests/test_agentic_entrypoint.py`); approval identity and handoff-without-approval refusals (`tests/test_workflow.py`, `tests/test_auth.py`). **Lost** at the time: the tainted-inventory injection scenarios as a suite. **Rebuilt 2026-09-01** on the product path as `services/evaluation_adversarial.py` (D17, `examples/run_adversarial_tool_loop.py`, gated by `tests/test_adversarial_tool_loop.py`): tainted hotel text, fabricated references, write-tool calls, identity substitution, injected user messages, and the three escalation routes. |
