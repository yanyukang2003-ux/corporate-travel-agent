...
- **§30.10 多城方案六步** —— 正在做，01–04 已落地（见 §32–§35），**下一步是第 05 步（唯一要花钱的一步）**
...
| 多城（三段以上） | ❌ 仍会被正确拦住不搜。**后端整条链路已经准备好**：规划器算得出三段（§33）、搜索侧搜得到三段与每站住宿（§34）、城市名认得出来（§35）。**只剩语义层还只出 1–2 段与一处住宿**，所以今天必须手工写死 `journey` 与 `stays`。见 §35.8 |
...
```
pytest 710 / ruff（src tests examples migrations）   全过（2026-08-29，§35）
...
| 05 | **语义层出航段列表** | ⬜ 下一步 **要花钱** | 开工前先花约 $0.05 量一次模型在三段上的抽取正确率，见 §35.8 |
...
*本轮见 §35（城市登记表）；上一轮 §34（搜索侧按段循环 + 每站住宿）；再上 §33、§32、§31；再上 §29；再上 §28、§27、§26、§25；A–F 见 §19；G 见 §20；H 见 §21；I 见 §22。*
