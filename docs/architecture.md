...
| 能力 | LLM | 确定性代码 |
|---|---:|---:|
| 把对话翻译成搜索参数（航段、最晚到达、逐字引用的日期原话） | 是 | 每个工具入口逐项校验，日期出处不成立即拒绝 |
| 决定下一步动作（再搜、换参数、提问、交付、判越界） | 是 | 预算记账、重复搜索拒绝、城市名标准化 |
| 声明硬要求与偏好（限定词表） | 是 | 按硬要求过滤、按偏好与习惯排序 |
| 行程组合、成本和时间 | 否 | 是 |
| 合规结论 | 否 | 是，且在库存交给模型之前贴好 |
| 交付哪些方案、推荐理由与未决问题 | 是，理由为自由文本 | 引用的 `ref_id` 必须来自本轮真实搜索；`explanation_facts` 由规划器生成、随方案展示，不经模型 |
| 审批有效性、幂等和交接 | 否 | 是 |
| 订后把变更原话读成变更请求 | 是，日期须逐字引用原话 | 影响评估、开改期任务、取消 |
| 员工习惯画像（从历史推偏好） | 否 | 是 |

模型的输出只有工具调用及其参数（OpenAI 兼容 function calling，`tool_choice=auto`）；工具表是白名单，没有任何写操作。参数进入领域层前由各工具入口逐项校验，校验不过只让这一次调用失败并把原因交回模型。
...
- 规划器生成、随方案一起展示给旅行者的 `explanation_facts`（模型的推荐理由另在 `propose_options.summary`，不改写这些事实）。
