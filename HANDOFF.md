...
- **§30.10 多城方案六步** —— 正在做，01、02 已落地（见 §32、§33），**下一步是第 03 步**
...
| 多城（三段以上） | ❌ 仍会被正确拦住不搜。**规划器已经能规划三段**（§33 第 02 步，闸门测试 11 条），但**搜索侧只搜两次、语义层只出 1–2 段**，真实链路到不了三段。四道形状拦路已拔掉两道，见 §33.5 |
...
```
pytest 689 / ruff（src tests examples migrations）   全过（2026-08-29，§33）
...
- 前端 build/test/lint 自 §33 起没再跑过（`types.ts` 只加了一个字段，但没跑就是没跑）
...
| 03 | **搜索侧按段循环 + 每站住宿** | ⬜ 下一步 | `agent/orchestrator.py`；见 §33.7 的两个前置问题 |
| 04 | 城市别名表 | ⬜ | `config/travel-policy.json`；也是 §1.5 第 1 条、§31.7 第 2 条 |
...
*本轮见 §33（结果侧变成航段列表）；上一轮 §32（多城方案 + 规划器去笛卡尔积）；再上 §31；再上 §29；再上 §28、§27、§26、§25；A–F 见 §19；G 见 §20；H 见 §21；I 见 §22。*
