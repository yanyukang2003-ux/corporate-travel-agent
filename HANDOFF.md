...
**分支：** `semantic-entrypoint-and-judge`（工作树不干净：工具循环入口未提交）  
**目的：** 换 session 续作入口。**读 §1、§30 和 §38 就能接上**，其余章节是历史记录，按需查。
...
**还欠一笔钱没花：** LLM Judge 60 条至今没重跑，**已经隔了 v12、v13、以及工具循环提示词
`tool-loop-v3`**。v11 的 4.55 早已作废。见 §37.7。
...
```
pytest 791 / ruff（src tests examples migrations）   全过（2026-08-30，§38）
...
**未在 tool-loop-v3 下重跑，引用前请注意：**
- LLM Judge 60 条（v11 下均分 4.55；已隔 v12、v13、工具循环三轮提示词）
- 工具循环日历/多城计费 runner（`run_tool_loop_*_evaluation.py`）旧目录是 v1 提示词，不能直接比
...
1. **给新增城市定夜费上限** —— 城市别名表本身已经在 §35 解决（五张表收成一张，
   ...
   仍然只有 10 座城市，广州、深圳、成都、杭州搜得到票也搜得到房，却因为查不到上限
   一律落 `INSUFFICIENT_EVIDENCE`、被降到最差档。
   **这个数字只有公司说得出**——时区和 IATA 码我可以查证，"广州一晚最多住多少钱"不行。
   登记表刻意把地理事实和政策决定分开存，就是为了这件事，见 §35.7
...
- **解释里还没写「落地 ≠ 到办事地点」。**
- 工具循环日历/多城计费 runner **没有**在 v3 下重跑。旧目录是 v1，不能直接比。
- 语义入口的 v13d 日期/多轮数字仍然有效，但那是**另一条链路**。

下一步工程上可做、且不必等拍板的：工具循环 runner 新开报告目录重跑；把「空段说明」写进给用户看的方案摘要，而不是只放在追问里。
...
**新 session 读三节：§1 现状，§30 架构方案，§38 工具循环出口。** 其余是历史记录，按需查。
**§30 是常读章节；产品入口和出口自由度以 §38 为准。**

*沟通标准见 `AGENTS.md`：先解释名词再用，先给结论再给细节，诚实优先于漂亮。*
*本轮见 §38（工具循环 Codex 形状）；上一轮 §37（一次多段报价）；再上 §36–§31；再上 §29；A–I 见 §19–§22。*
