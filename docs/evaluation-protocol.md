...
门禁只设在改变旅行者结果的层：终局决策类别、搜索的路线与到达日、硬要求。偏好只改排序，只报告。
**四层跟着工具表走**：它们是当前工具表的决策点，工具表变了层要重定。**调用次数不判相同**：同一条用例三轮各花了
几次模型调用 / 搜索 / 被宿主拒掉 / 几轮对话，按离散度（最大减最小）报告，超阈值列出，不设门禁。
一致性是探测器，正确才是目标：每条不一致归到三个桶里（模型不稳、规则没定、判据噪声）再决定修什么。
...
搜索参数去重后一致 139/143、追问核心事实一致 139/177；D18 终局一致 290/300。带第 3 层的 2026-09-06
185 × 3（`consistency-external-longtail-multiturn-3x-v2-20260906/`）：终局 174/185、搜索 138/143、
硬要求 129/137、偏好 85/137、追问核心 136/179；终局摇摆 11 条里 6 条是硬要求（`train_only`）翻转。
...
| 指标 | 口径 |
|---|---|
| `handoff_completion_rate` | 系统给出过方案的任务里，员工点了"已在官方平台订好"的比例 |
| `seconds_to_handoff_*` | 任务开始 → 交接完成的**墙上时间**（均值 / P50 / P95） |
| `advance_booking_days_*` | 交接时刻 → 首段实际出发（均值 / P50） |
| `policy_violation_rate.selected` | 选中方案里带"需审批 / 禁止"证据的比例 |
| `policy_violation_rate.offered` | 展示出去的方案里带同样证据的比例 |
| `approval_request_rate` / `clarification_rounds_mean` / `tool_calls_per_task_mean` | 例外审批率、平均追问轮数、每任务工具调用数 |

三条必须说在前面的限制：

1. **交接完成率不是真实的渠道内预订率。** 分子是员工的自述，不是供应商回执；
   在接到真实预订回执之前只能当**上限估计**。
2. **耗时是墙上时间**，含员工去开会、经理隔天才批。离线评测里它量的是跑批脚本
   有多快，跨跑次比较没有意义。系统自身耗时看 6.6。
3. **超标发生率实际上量的是"需要审批"的比例**：按已编码不变量，`FORBIDDEN`
   方案在规划阶段就被硬过滤，根本不会展示。

分母为零时状态是 `unavailable`，值为 `null`，**不写成 0**。

**时钟**：审计时间戳走真实墙上时间，不是编排器时钟。冻了时钟的跑法必须把冻住的
时刻作为 `handoff_reference_time` 传进去，否则提前预订天数是两条时间线相减，没有意义。

**这一层还没有门禁。** 第 8 节的门禁表暂不收录它——门禁要建立在真实数据的基线上，
现在还只有演示数据（`reports/evaluation-runs/business-outcomes-demo-20260831/`）。
